...
def build_txn(wallet, txn_calldata):
    try:
        ...
        print(f'Error in (Trusta/attestA: build_txn) {ex.args}')


def attest_a(wallet):
    try:
        print(f'Attesting Trusta Group A')
        ...
        print(f'Error in (Trusta/attestA: attest_a) {ex.args}')


def score_check(wallet):
    try:
        print(f'Checking attestation score Trusta Group A')
        token_auth = sign_in_message(wallet)
        txn_calldata = get_attest_data_humanity(token_auth)
        score = txn_calldata['message']['score']
    except Exception as ex:
        print(f'Error in (Trusta/attestA: score_check) {ex.args}')
